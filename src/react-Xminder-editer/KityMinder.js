...
class KetyMinder extends React.Component {
  ...
  componentDidMount = () => {
    ...
    if (
      ...
      this.props.editor.indexOf(this.userName) === -1
    ) {
      this.disableMinder();
    }
  ...
  componentWillReceiveProps = (nextProps) => {
    ...
    if (
      nextProps.type === "" &&
      JSON.stringify(nextProps.editor) !== JSON.stringify(this.props.editor)
    ) {
  ...
  sendPatch = (diff) => {
    ...
    this.sendMessage(
      "1" + JSON.stringify({ case: this.getAllData(), patch: [diff] })
    );
  };
  ...
  onResultChange = () => {
    ...
    if (
      this.state.searchDrawerVisible &&
      this.props.type === "record" &&
      this.recordStatusSearchRef.state.recordStatus !== ""
    ) {
      this.recordStatusSearchRef.handleChange(
        this.recordStatusSearchRef.state.recordStatus
      );
    }
  };
  ...
  render() {
    const operations = (
      <React.Fragment>
        ...
        <span style={{ color: this.state.websocketStatus === 0 ? "red" : "" }}>
          {this.state.websocketStatus === 0 &&
        ...
        >
          <Icon
            type="double-left"
            style={{
              transform: this.state.expand ? "rotate(90deg)" : "rotate(-90deg)",
            }}
          />
    ...
    return (
      <div
        id="kityminder-editor"
        style={this.props.editorStyle}
        className="kityminder-editor-container kityminder-editor"
      >
        ...
        />
        {
          <Tabs
            defaultActiveKey={
              this.props.type === "compare" ? "exterior" : "mind"
            }
            size="small"
        ...
        <div
          id="kityminder-core"
          tabIndex={-1}
          className="kityminder-core-container focus"
          ref={(input) => {
            if (!this.minder) {
        ...
        >
          ...
          >
            <Input.TextArea
              ...
              onPaste={(e) => this.handleEditPaste(e)}
